...
const ProductDetails = ({ product, onClose, language }) => { // Renamed back to ProductDetails to match import
    ...
    const [isFavorite, setIsFavorite] = useState(product.favourite || false); // Initialize from prop
    ...
    useEffect(() => {
        if (productDetails && !loadingProductDetails) {
            ...
            setIsFavorite(productDetails.product?.favourite || false); // Update from fetched data

            // Initialize addons with quantity
            const initialAddons = {};
            ...
            // Initialize extras with quantity 0
            const initialExtras = {};
    ...
    const handleFavoriteToggle = async () => {
        ...
        const success = await changeState(url, isFavorite ? `${product.name} Removed from favorites` : `${product.name} Added to favorites`, { favourite: newFavoriteState === true ? 1 : 0, });
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                ...
                <div className="p-6">
                    ...
                    <div className="mb-6 p-3 bg-gray-50 rounded-lg">
                        ...
                        {taxSetting === 'included' && displayData.tax_val > 0 && (
                            <div className="text-sm text-gray-600 mt-1">
                                Tax included: {displayData.tax_val} EGP
                            </div>
                        )}
                    </div>
                            ...
                            <h3 className="font-semibold mb-3">
                                ...
                                {variation.type === 'multiple' && (
                                    <span className="text-sm text-gray-500 ml-2">
                                        (Select {variation.min}-{variation.max})
                                    </span>
                                )}
                            </h3>
                                            ...
                                            {currentAddon?.checked && (
                                                <div className="mt-2 flex items-center justify-end gap-2 pl-6">
                                                    {canChangeQuantity ? (
                                                        ...
                                                        <span className="text-sm text-gray-600">Quantity: 1 (fixed)</span>
                                                    )}
                                                </div>
                                            )}
                                        </div>
                    ...
                    {/* Extras - Only show available ones */}
                    {availableExtras.length > 0 && (
                        <div className="mb-6">
                            <h3 className="font-semibold mb-3">Available Extras</h3>
                                        ...
                                        <div key={extra.id} className="p-3 border rounded-lg hover:bg-gray-50">
                                            ...
                                            <div className="flex items-center justify-between">
                                                <span className="text-sm text-gray-600">
                                                    {min > 0 && `Min: ${min}, `}Max: {max === Infinity ? 'No limit' : max}
                                                </span>
                                                <div className="flex items-center space-x-2">
                                                    <button
                    ...
                    {displayData.excludes?.length > 0 && (
                        <div className="mb-6">
                            <h3 className="font-semibold mb-3">Exclude Items</h3>
                            <div className="space-y-2">
                                {displayData.excludes.map((exclude) => (
                                    <label key={exclude.id} className="flex items-center p-3 border rounded-lg hover:bg-gray-50 cursor-pointer">
                                        <input
                                            type="checkbox"
                    ...
                    >
                        {canAddToCart() ? 'Add to Cart' : 'Please complete selection'}
                    </button>
                </div>
            </div>
        </div>
